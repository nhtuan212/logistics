<?php
	...
	function get_photos()
	{
		...
		$sql = "select * from #_gallery where $where order by number,id desc $limit";
		$items = $d->rawQuery($sql,array($idc,$com,$type,$kind,$val));
		$sqlNum = "select count(*) as 'num' from #_gallery where $where order by number,id desc";
	...
	function get_photo()
	{
		...
		$item = $d->rawQueryOne("select * from #_gallery where id_photo = ? and com = ? and type = ? and kind = ? and val = ? and id = ? order by number,id desc limit 0,1",array($idc,$com,$type,$kind,$val,$id));
	...
	function save_photo()
	{
		...
		if($id)
		{
			if(isset($_FILES['file']))
			{
				...
				{
					$data['photo'] = $photo;
					$row = $d->rawQueryOne("select id, photo from #_gallery where id_photo = ? and com = ? and type = ? and kind = ? and val = ? and id = ? order by number,id desc limit 0,1",array($idc,$com,$type,$kind,$val,$id));
			...
			if(isset($_FILES['file-taptin']))
			{
				...
				if($taptin = $func->uploadImage("file-taptin", $config[$com][$type][$dfgallery][$val]['file_type_photo'],UPLOAD_FILE,$file_name."-taptin"))
				{
					...
					$row = $d->rawQueryOne("select id, taptin from #_gallery where id_photo = ? and com = ? and type = ? and kind = ? and val = ? and id = ? order by number,id desc limit 0,1",array($idc,$com,$type,$kind,$val,$id));
