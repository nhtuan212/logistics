...
<?php if(@$template != 'index' && @$template != 'user/login') { ?>
        ...
        $(document).ready(function(e) {
            ...
            $("body").on("click", "a.check-attr", function(){
                ...
                $.ajax({
                    ...
                    data:{id:id, tbl:tbl, attr:attr, action:"status"},
                });
            });
            ...
            $("body").on("change", ".onchange-number input", function(){
                ...
                if(id != "")
                {
                    $.ajax({
                        type: "POST",
                        url: "ajax/ajax_attr.php",
                        data:{id:id, tbl:tbl, attr:attr, value:value},
                    });
                }               
            });
    ...
    <?php if(@$config[$com][$type]['tags']=='true') { ?>
        ...
        <script type="text/javascript">
            $(document).ready(function(e) {
                window.asd = $('.SlectBox').SumoSelect({csvDispCount: false,});
            });
        </script>
    <?php } ?>
            ...
            $(document).ready(function(e) {
                ...
                $("body").on("change", ".update-gallery", function(){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "ajax/ajax_attr.php",
                        data:{tbl:tbl, id:id, attr:attr, value:value},
                    });
                });
                    ...
                    var sortable = Sortable.create(sortGallery, {
                        ...
                        onEnd: function(){
                            ...
                            $.ajax({
                                ...
                                dataType: 'json',
                                data:{'id': '<?=@$_GET['id']?>', 'idGallery_arr': idGallery_arr, 'type': '<?=@$_GET['type']?>', 'level': '<?=@$_GET['level']?>'},
                                success:function(rs){
                                    if(rs)
                                    {
                                        stt = rs.stt;
...
<script type="text/javascript">
    ...
    function ajax_category(tbl, attr, type, level)
    {
        ...
        if(lv_next>0)
        {
            $.ajax({
                ...
                type: "POST",
                dataType: 'html',
                data: {tbl:tbl, id:id, attr:attr, type:type, level:level},
                success: function(rs){
    ...
    function ajax_place(tbl, attr, title, tbl_find="")
    {
        ...
        if(tbl_find != '')
        {
            $.ajax({
                ...
                type: "POST",
                dataType: 'html',
                data: {tbl:tbl, id:id, attr:attr, title:title},
                success: function(rs){
                    $(".id_dist").html(rs);
                }
            });
    ...
    function status_update(id)
    {
        ...
        if(id > 0 && id_status > 0)
        {
            $.ajax({
                type: "POST",
                url: "ajax/ajax_attr.php",
                data: {id:id, value:id_status, attr:attr, tbl:tbl},
                success: function(rs){
    ...
    function order_update(id, id_order)
    {
        ...
        $.ajax({
            ...
            dataType: 'json',
            data: {id:id, id_order: id_order, qty: qty, act: 'update'},
            success: function(rs){
                $('.total').html(rs.total);
            }
        });
            ...
            function checkSlug(type) {
                ...
                if(slug)
                {
                    $.ajax({
                        ...
                        type: "POST",
                        dataType: 'json',
                        data: {id:id, name:name, slug:slug, type:type, act:act},
                        success: function(rs){
                            var slug = $("#slug");
                            if(type == 'check')
                            {
