<?php
	...
	@define ( 'DIR' , UPLOAD_POST);
	@define ( 'DIR_CATEGORY' , UPLOAD_POST_L);
	...
	@$tbl = empty($_REQUEST['level']) ? 'post' : 'category';
	...
	function get_item()
	{
		global $func, $d, $config, $tbl, $post, $totalRows , $pageSize, $offset, $paging, $type;
		foreach($config['website']['lang'] as $lang => $value)
		{
			foreach($config['post'][$type]['text'] as $column => $value)
			{
		...
		for($i=0; $i < $config['post'][$type]['level']; $i++) { 
		...
		$pageSize = 15;
		$bg = $page * $pageSize;
		$post = $d->rawQuery("select * from #_$tbl where id<>0 $where limit $bg,$pageSize");	
		$paging = $func->paging($totalRows, $pageSize);
	}
	function edit_item()
	{
		global $func, $d, $tbl, $item, $item_seo, $info_options, $link, $type, $error_404;
		...
		$info_options = (isset($item['info_options']) && $item['info_options'] != '') ? json_decode($item['info_options'],true) : null;
	...
	function save_item()
	{
		...
		if($id && $act != "save_copy")
		{
			$post = $d->rawQueryOne("select photo, file from #_$tbl where id=?", array($id));
			if(@$data['photo'] && $post) $func->delete_file(DIR.$post['photo']);
			if(@$data['file'] && $post) $func->delete_file(UPLOAD_FILE.$post['file']);

			if($config['post'][$type]['slug']==true)
			{
	...
	function delete_item()
	{
		...
		if($id)
		{
			$post = $d->rawQueryOne("select id, photo, file from #_$tbl where id=?", array($id));
			$gallery = $d->rawQuery("select id, photo from #_gallery where id_parent=? and level=0", array($id));

			$func->delete_file(DIR.$post['photo']);
			$func->delete_file(UPLOAD_FILE.$post['file']);
